    ...
    with open(log_file_path, 'r') as log_file:
        for line in log_file:
            if line.startswith("Date:"):
                ...
                active_sessions[current_user] = {"logout_time": logout_time, "login_time": None, "workstation": workstation_name}
            elif "Logged into Client Module" in line:
                current_user = line[14:44].strip()
                login_time = line[5:13].strip()
                message = line[45:].strip()
                workstation_name = extract_workstation_name(message)
                if current_user in active_sessions and active_sessions[current_user]["login_time"] is None:
                    active_sessions[current_user]["login_time"] = login_time
                    duration = calculate_duration_in_minutes(active_sessions[current_user]["login_time"], login_time)
